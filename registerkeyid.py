...
def get_email_and_code_fetcher(proxies: Any = None):
    public_key, private_key = generate_keypair_hex()
    ...
    def fetch_code(timeout_sec: int = 180, poll: float = 6.0) -> str | None:
        ...
        while time.monotonic() - start < timeout_sec:
            ...
            try:
                ...
                print(f"[otp] 轮询 #{attempt}, 收到 {len(messages)} 封邮件, 目标: {email}")
                for m in messages:
                    # 优先在 body 或 extractedText 中查找
                    body_text = str(m.get("body", "")) + str(m.get("extractedText", ""))
            ...
            time.sleep(poll)
        return None
    return email, _gen_password(), fetch_code, public_key, private_key
...
def _remove_account_entry(accounts_path: Path, email: str):
    ...
    try:
        ...
        kept = [ln for ln in lines if not ln.strip().startswith(f"{email}----")]
...
def run(proxy: Optional[str]):
    ...
    try:
        ...
        print("[步骤1] 正在通过 KeyID 获取本地化邮箱...")
        email, password, code_fetcher, public_key, private_key = get_email_and_code_fetcher(proxies)
        ...
        for i in range(6):
            ...
            if "code=" in next_url and "state=" in next_url:
                ...
                return token_json, email, password, public_key, private_key
...
def main():
    ...
    while True:
        ...
        if res:
            ...
            token_json, email, real_pwd, pub_key, priv_key = res
            ...
            with open(os.path.join("tokens", "accounts.txt"), "a", encoding="utf-8") as f:
                # f.write(f"{email}----{real_pwd}\n")
                f.write(f"{email}----{real_pwd}----{pub_key}----{priv_key}\n")
            ...
            if upload_ok and args.cpa_prune_local:
                ...
                _remove_account_entry(tokens_dir / "accounts.txt", email)
